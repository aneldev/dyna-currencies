...
import {DynaCurrencies, IDynaLabelCurrency, ICurrencyRates, ICurrency} from './../src';
...
describe('DynaCurrencies test', () => {
  it('should load the rates', () => {
    dynaCurrencies.update(rates);
    expect(dynaCurrencies.count).not.toBe(0);
  });

  it('should generate a label with values', () => {
    let label: IDynaLabelCurrency = dynaCurrencies.convertToLabel(1, 'euR', 'Gbp');
